...
class AdminPasswordResetController extends Controller
{
        ...
        try {
            ...
            if (!$existingUser) {
                return response()->json([
                    'code' => 'not-exist',
                    'message' => 'If Account exists, an email to reset your password has ben sent', //this message is intentional, although account does not exist
                ]);
            }
            ...
            $resetLink = env('FRONTEND_URL') . '/admin/accounts/password/reset/reset-password/' . $passwordResetToken;
    ...
    public function AdminResetPassword(Request $request){
        ...
        $request->validate([
            ...
            'newPassword' => ['required', 'string', 'regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{6,}$/']
        ]);
        try{
            $fetchUser = Admin::where('email', $email)->first();
            if(!$fetchUser){
                return response()->json([
